...
raw={"id":"1789650362413-0405027a","text":"  cold water  ","kind":"change","name":"Resident","received_ts":1789650362413,
     "context":{"soc":97.5,"stale":["soc"],"evil":"<script>","page":"/d.html"},"site":"SPOOFED"}
n=fleet.clean_note(st,raw)
check("note cleaned: text trimmed, site comes from OUR list not the payload", n["text"]=="cold water" and n["site"]=="Site B" and n["key"]=="site-b/1789650362413-0405027a")
...
other={"key":"site-a","name":"Site A","state_url":"http://d:8780/state.json"}
newer=dict(raw, received_ts=raw["received_ts"]+5000, text="newer")
check("same note id at another site is a different note; newest first", fleet.merge_inbox(inbox,other,{"notes":[newer]})==1 and inbox[0]["site"]=="Site A" and len(inbox)==2)
